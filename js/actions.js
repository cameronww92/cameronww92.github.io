// var TimerElement = document.getElementById('timer-wrapper');
var TimerElement = document;
var ham = new Hammer(TimerElement);
...
ham.on("press", function(e) {
	let contentElem = document.getElementById('page-content-wrapper');
	contentElem.style.opacity = 1.0;
	contentElem.style.visibility = "visible";
	menuOpen = true;


	var BGElement = document.getElementById('page-content-background');
	var bgHam = new Hammer(BGElement);
	bgHam.on("tap", function(e) {
		contentElem.style.opacity = 0.0;
...
Actions.prototype.init = function()
{
	...
	ham.on("tap", this.handle_gesture.bind(this));
...
Actions.prototype.load_timer = function(timer)
{
	...
	//Setting timer title
	$("#run-title").text(timer.timer_name);
...
Actions.prototype.handle_gesture = function(ev)
{
	if(this.get_page() == "timer-control" && !menuOpen)
	{
		switch(ev.type)
		{
			case "tap":
				// console.log("tap start timer");
				ev.preventDefault();
				this.timer_start_split();
				break;
		}
